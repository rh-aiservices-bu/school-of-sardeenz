"""FastAPI app exposing the engine-runner contract on top of an MLServer subprocess."""
...
def _engine_version() -> str:
    try:
        import mlserver  # type: ignore

        return str(getattr(mlserver, "__version__", "unknown"))
    except Exception:
        return "unknown"


async def _health_poller(app: FastAPI) -> None:
    """Flip STARTING → READY once MLServer serves, then keep watching: flip → ERROR if the
    subprocess dies at any point (a post-startup crash must not leave the runner reporting READY
    forever)."""
    engine: MLServerEngine = app.state.engine
    status: st.RunnerStatus = app.state.status
    client: httpx.AsyncClient = app.state.client

    # Startup phase.
    status.set_progress(st.PHASE_LOADING_WEIGHTS, 10, "Starting MLServer engine")
    while status.state == st.STARTING:
        if not engine.is_alive():
            status.mark_error("MLServer engine process exited during startup")
            return
    ...
    while True:
        ...
        if not engine.is_alive():
            if status.state in (st.READY, st.BUSY, st.SLEEPING):
                status.mark_error("MLServer engine process exited")
            return
...
def create_app(args: RunnerArgs) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        model_settings = build_model_settings(args.model, args.served_name)
        repo_dir = write_model_repository(model_settings)
        app.state.engine = MLServerEngine(args, repo_dir)
    ...
    app = FastAPI(title="Sardeenz MLServer Runner", lifespan=lifespan)
    ...
    async def post_sleep(request: Request) -> JSONResponse:
        ...
        try:
            await app.state.engine.sleep(app.state.client)
        except httpx.HTTPError as exc:
            return _error(500, "SLEEP_FAILED", f"MLServer sleep failed: {exc}")
    ...
    async def post_wake() -> JSONResponse:
        try:
            ...
            return _error(500, "WAKE_FAILED", f"MLServer wake failed: {exc}")
        status().mark_awake()
