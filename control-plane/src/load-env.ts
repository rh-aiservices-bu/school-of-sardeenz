 ...
 * it finds. Variables already present in the environment are never overridden, so container/k8s
 * config (and per-process overrides such as `SARDEENZ_CONTROL_PLANE_LISTEN_ADDR=… npm run dev`)
 * always win over the file. No-op in production (`NODE_ENV=production`) and when no `.env` exists,
