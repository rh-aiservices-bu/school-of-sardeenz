 ...
 * config (and per-process overrides) always win over the file. No-op in production
 * (`NODE_ENV=production`) and when no `.env` exists, so it is safe to call unconditionally at
 * startup.
 *
