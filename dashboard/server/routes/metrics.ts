...
export function registerMetricsRoutes(app: FastifyInstance, deps: RouteDeps): void {
  ...
  app.get('/api/metrics/connections', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const start = q['start'] ?? defaultStart();
    const end = q['end'] ?? defaultEnd();
    const step = q['step'] ?? DEFAULT_STEP;
  ...
  app.get('/api/metrics/parking-duration', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const start = q['start'] ?? defaultStart();
    const end = q['end'] ?? defaultEnd();
    const step = q['step'] ?? DEFAULT_STEP;
  ...
  app.get('/api/metrics/wake-triggers', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const result = await deps.prometheus.queryRange(
  ...
  app.get('/api/metrics/state-transitions', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const result = await deps.prometheus.queryRange(
  ...
  app.get('/api/metrics/evictions', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const result = await deps.prometheus.queryRange(
  ...
  app.get('/api/metrics/memory-history', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const result = await deps.prometheus.queryRange(
  ...
  app.get('/api/metrics/operations', { preHandler: [app.authenticate, app.requireRole('admin-readonly')] }, async (request, reply) => {
    const q = request.query as Record<string, string>;
    const start = q['start'] ?? defaultStart();
    const end = q['end'] ?? defaultEnd();
    const step = q['step'] ?? DEFAULT_STEP;
