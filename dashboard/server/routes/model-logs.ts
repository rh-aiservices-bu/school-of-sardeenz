...
/**
 * Streams legacy live model logs or durable instance startup logs to the browser.
 *
 ...
 * frames from the selected control-plane log endpoint reach the browser `EventSource`
 * unmodified.
 *
...
export function registerModelLogRoutes(app: FastifyInstance, deps: RouteDeps): void {
  const registerStream = (
    path: string,
    controlPlanePath: (name: string, instanceId?: string) => string,
  ): void => {
    app.get<{ Params: { name: string; instanceId?: string } }>(
      path,
      { preHandler: [app.authenticate, app.requireRole('admin-readonly')] },
      async (request, reply) => {
        const { name, instanceId } = request.params;

        let upstream: Response;
        try {
          upstream = await deps.controlPlane.proxyRequest(
            'GET',
            controlPlanePath(name, instanceId),
          );
        } catch (err) {
          app.log.error(
            { err, modelName: name },
            'Control plane unreachable for model logs stream',
          );
          return reply
            .code(502)
            .send({ error: 'Control plane unreachable', code: 'UPSTREAM_ERROR' });
        }
  ...
  registerStream(
    ...
    (name) => `/api/v1/models/${encodeURIComponent(name)}/logs`,
  );
  registerStream(
    '/api/models/:name/instances/:instanceId/startup-logs',
    (name, instanceId) =>
      `/api/v1/models/${encodeURIComponent(name)}/instances/${encodeURIComponent(instanceId ?? '')}/startup-logs`,
  );
}
