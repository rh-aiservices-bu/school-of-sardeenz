...
vLLM's structured output API changed significantly across versions (guided_* deprecated and removed, backend flags renamed, new parameters added). A proxy that inspects these parameters would need constant updates. A pure passthrough proxy is immune to this churn.
